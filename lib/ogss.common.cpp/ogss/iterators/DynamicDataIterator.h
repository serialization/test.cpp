...
namespace ogss {
    ...
    namespace iterators {
        ...
        public:
            ...
            DynamicDataIterator() : p(nullptr), endHeight(0), index(0), last(0), second(false) {}

            explicit DynamicDataIterator(const Pool<T> *p)
                    : p(p), endHeight(p->THH),
                      index(p->bpo), last(index + p->cachedSize),
                      second(false) {
            ...
            DynamicDataIterator<T> begin() const {
                return *this;
            }

            DynamicDataIterator<T> end() const {
                return DynamicDataIterator<T>();
            }
        };
    }
}
