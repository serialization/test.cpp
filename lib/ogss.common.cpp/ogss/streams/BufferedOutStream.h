...
namespace ogss {
    namespace streams {
         ...
         *
         * @note in contrast to OGSS/JVM, buffers will not be recycled because C++ stdlib lacks the required data structures
         * @note in contrast to OGSS/JVM, we can directly write bools to the last byte, what makes BoolWrapper pointless
         */
        class BufferedOutStream {
            ...
            struct Buffer {
                ...
                //! used to restore begin; if negative, the buffer is wrapped, i.e. begin must not be freed
                const int size;
            };
            ...
            /**
             * Offset of last boolean operation. We start with a full byte, because the first last byte has to be the
             * absolute first byte.
             *
             * @note this implementation is sane because a buffer containing bool cannot contain any other entity.
             */
            int8_t off = 7;
        ...
        public:
                                ...
                                if (v < 0x800000000U) {
                                    ...
                                    *(current.begin++) = (uint8_t) ((0x80U | v >> 28u));
                                    if (v < 0x40000000000U) {
                                        ...
                                        *(current.begin++) = (uint8_t) ((0x80U | v >> 35u));
                                        if (v < 0x2000000000000U) {
                                            *(current.begin++) = (uint8_t) ((v >> 42u));
                                        } else {
                                            *(current.begin++) = (uint8_t) ((0x80U | v >> 42u));
                                            if (v < 0x100000000000000U) {
                                                *(current.begin++) = (uint8_t) ((v >> 49u));
                                            } else {
                                                *(current.begin++) = (uint8_t) ((0x80U | v >> 49u));
                                                *(current.begin++) = (uint8_t) ((v >> 56u));
                                            }
                                        }
                                    }
                                }
            ...
            void put(const uint8_t *data, const size_t size) {
                // write the byte[] directly, if it is too large to be cached efficiently
